# @NCLEXPrePro — 14 Tweets Ready to Paste into Buffer
# Copy each tweet block → paste into Buffer → queue for 7pm CST daily
# Takes ~5 minutes total

---

## TWEET 1 — Monday
```
ABCs save lives on NCLEX and in real life.

Every prioritization question: ask yourself who's dying first.

Airway → Breathing → Circulation

If an answer involves any of those 3 — that's your pick.

🩺 Test your clinical judgment free → https://nclexprepro.com/sample
#NCLEX #NursingStudents
```

---

## TWEET 2 — Tuesday
```
NCLEX pharm tip that changed everything:

Stop memorizing individual drugs.
Learn the SUFFIX instead.

-olol = beta blocker (slow the heart)
-pril = ACE inhibitor (lower BP, dry cough)
-statin = cholesterol (watch for muscle pain)
-mycin = antibiotic (watch for ototoxicity)

12 suffixes covers 80% of NCLEX pharm. 💊
#NCLEX #Pharmacology
```

---

## TWEET 3 — Wednesday
```
The real reason people fail NCLEX:

They study what they know instead of what they don't.

Track every wrong answer. Review the rationale.
Ask: "Why is this wrong?" not just "what's right?"

That shift alone moves the needle more than 100 extra practice questions.
#NursingSchool #NCLEX
```

---

## TWEET 4 — Thursday
```
SATA tip: treat each option like true/false.

Don't think "I need 3 answers."
Think: "Is option A true? Yes or no."
Then B. Then C.

They're independent questions, not a group puzzle.

This mindset shift = more SATA correct answers. ✅
#NCLEX
```

---

## TWEET 5 — Friday
```
Fluid & electrolytes made simple:

SALT = Sodium

LOW sodium → water moves INTO cells → brain swells
HIGH sodium → water moves OUT of cells → confusion/seizure

All the symptoms flow from that one principle.
#NCLEX #NursingStudents
```

---

## TWEET 6 — Saturday
```
Quick poll for nursing students:

What's your hardest NCLEX topic?

🔴 Prioritization
🟡 Pharmacology
🟢 SATA
🔵 Mental Health

Drop your answer below 👇
#NCLEX #NursingSchool
```

---

## TWEET 7 — Sunday
```
If you're studying for NCLEX right now, you're already doing the hard part.

The exam is hard. Nursing school is hard.
But you didn't come this far to only come this far.

Free 10-question practice → https://nclexprepro.com/sample

You've got this. 🩺
#NursingStudents #NCLEX
```

---

## TWEET 8 — Monday Week 2
```
Therapeutic communication on NCLEX = 4 rules:

1. Never give advice ("you should...")
2. Never minimize ("don't worry...")
3. Never use yes/no questions
4. Always reflect the patient's feelings back

When stuck → pick the answer that makes the patient feel heard.
#NCLEX #MentalHealth
```

---

## TWEET 9 — Tuesday Week 2
```
Most underrated NCLEX study strategy:

Read the LAST line of the question first.

"What should the nurse do NEXT?"
"Which finding requires IMMEDIATE action?"
"What is the PRIORITY intervention?"

The last line tells you exactly what kind of answer to look for.
#NCLEXTips #NursingStudents
```

---

## TWEET 10 — Wednesday Week 2
```
Delegation rules for NCLEX:

RN → complex assessments, unstable patients, teaching
LPN → stable patients, routine care
CNA → basic ADLs, vital signs on stable patients

When in doubt: if it requires judgment → RN only.
#NCLEX #Prioritization
```

---

## TWEET 11 — Thursday Week 2
```
Labs you MUST know for NCLEX:

Na+ → 135-145
K+ → 3.5-5.0 (dangerous outside this range)
Glucose → 70-110 fasting
INR → 2-3 therapeutic (warfarin)
PTT → 60-100 therapeutic (heparin)

Screenshot this. ✅
#NCLEX #LabValues
```

---

## TWEET 12 — Friday Week 2
```
Signs of increased ICP:

🔴 Cushing's Triad:
- Rising BP
- Slowing heart rate
- Irregular respirations

This is a neurological emergency.

See these 3 together on NCLEX → call the doctor immediately.
#NCLEX #MedSurg
```

---

## TWEET 13 — Saturday Week 2
```
Honest take: the best NCLEX prep isn't more content.

It's more practice questions with good rationales.

50 wrong answers with explanations > 200 right answers with no feedback.

Quality of review > quantity of questions.
#NCLEX #NursingSchool
```

---

## TWEET 14 — Sunday Week 2
```
2 weeks of NCLEX tips.

If even one of these helped — share this with a nursing student who needs it.

When you pass, come back and tell me. I love hearing those stories. 🎉

Free 10-question practice → https://nclexprepro.com/sample
#NCLEX #NursingStudents #YouGotThis
```

---

---

## TWEET 15 — Week 3 Monday
```
The 3 types of NCLEX questions you'll see:

1. Knowledge — do you know the fact?
2. Application — can you use the fact?
3. Analysis — can you figure out what the fact means for THIS patient?

Most people fail because they study for type 1 but the test asks type 2 and 3.

#NCLEX #NCLEXTips
```

---

## TWEET 16 — Week 3 Tuesday
```
Acid-base in 60 seconds:

pH low = acidosis | pH high = alkalosis
CO2 high = acidosis | CO2 low = alkalosis
HCO3 high = alkalosis | HCO3 low = acidosis

ROME: Respiratory Opposite, Metabolic Equal

CO2 and pH go opposite → respiratory
HCO3 and pH go same → metabolic

#NCLEX #MedSurg
```

---

## TWEET 17 — Week 3 Wednesday
```
The night before NCLEX:

❌ Don't cram new content
❌ Don't look at practice questions
❌ Don't stay up late

✅ Pack your bag
✅ Eat a real meal
✅ Sleep 7-8 hours

What you know the night before is what you'll walk in with. Protect it.

#NCLEX #NursingStudents
```

---

## TWEET 18 — Week 3 Thursday
```
Infection control priority order:

1. Airborne (TB, measles, varicella) → negative pressure + N95
2. Droplet (flu, COVID, pertussis) → surgical mask, 3 ft
3. Contact (C. diff, MRSA) → gloves + gown

When in doubt: the more protective option is always correct.

#NCLEX #InfectionControl
```

---

## TWEET 19 — Week 3 Friday
```
If you're retaking NCLEX, read this:

The exam didn't fail you. The strategy did.

More questions ≠ better score.
Better review = better score.

Know *why* each answer is wrong. That's the entire game.

🏥 Free 10-question sample → https://nclexprepro.com/sample

#NCLEX #NursingStudents
```

---

## TWEET 20 — Week 3 Saturday
```
Pain assessment tip for NCLEX:

Pain is always subjective. The patient's report IS the data.

"Patient says 8/10 but is laughing" → still 8/10.

The nurse's job is to believe and respond — not judge.

This shows up in prioritization questions constantly.

#NCLEX #MedSurg
```

---

## TWEET 21 — Week 3 Sunday
```
3 weeks in. Still showing up. That says everything.

NCLEX is a marathon. The students who pass aren't always the smartest.

They're the ones who kept going when it got hard.

That's you. Keep going. 💙

#NursingStudents #NCLEX #YouGotThis
```

---

## TWEET 22 — Week 4 Monday
```
Insulin types — fastest to slowest:

⚡ Lispro/Aspart → 15 min onset (mealtime)
🔵 Regular → 30-60 min onset
🟡 NPH → peaks 6-14 hrs (overnight hypo risk)
🟢 Glargine → no peak (basal)

The one with a peak = hypoglycemia risk while sleeping.

#NCLEX #Pharmacology
```

---

## TWEET 23 — Week 4 Tuesday
```
What "delegate to the UAP" means on NCLEX:

✅ Vital signs (stable patient)
✅ Ambulation assistance
✅ Bathing and hygiene

❌ Assessment
❌ Teaching
❌ IV meds
❌ Anything requiring nursing judgment

Stable + routine = UAP. Everything else = RN.

#NCLEX #Prioritization
```

---

## TWEET 24 — Week 4 Wednesday
```
NCLEX tip nobody talks about:

The stem of the question contains the answer.

Read it twice. Look for:
- Age and gender
- Diagnosis
- Timing (post-op day 1 vs day 5)
- What the question is actually asking

The answer is hiding in the details you skip.

#NCLEXTips #NursingSchool
```

---

## TWEET 25 — Week 4 Thursday
```
Post-op priorities by day:

Day 0-1: Airway, pain, bleeding, circulation
Day 2-3: Infection signs, mobility, bowel sounds
Day 4+: Teaching, discharge planning, wound healing

Most NCLEX post-op questions tell you the day number for a reason.

#NCLEX #MedSurg
```

---

## TWEET 26 — Week 4 Friday
```
Cardiac meds cheat sheet:

-olol (beta blockers) → slow HR, lower BP
-pril (ACE inhibitors) → vasodilate, watch for cough
-sartan (ARBs) → like ACE but no cough
Nitrates → vasodilate, watch for hypotension
Digoxin → slow + strengthen (toxic > 2.0)

Hold cardiac meds if HR < 60 unless ordered.

#NCLEX #Pharmacology
```

---

## TWEET 27 — Week 4 Saturday
```
The 5 rights of delegation:

1. Right task
2. Right circumstance
3. Right person
4. Right direction and communication
5. Right supervision

All 5 must be met. Miss one → don't delegate.

#NCLEX #Prioritization
```

---

## TWEET 28 — Week 4 Sunday
```
You've been studying. You've been showing up.

Here's your reminder: you ARE prepared.

The anxiety before NCLEX? That's your brain taking it seriously.

Channel it. Don't fight it.

Free practice → https://nclexprepro.com/sample

#NCLEX #NursingStudents
```

---

## TWEET 29 — Week 4 Monday
```
The last thing most nurses wish they'd known before NCLEX:

It's not about knowing more. It's about thinking differently.

NCLEX doesn't test memorization. It tests clinical judgment.

"What would a safe nurse do RIGHT NOW?"

Answer that — every time.

#NCLEX #NCLEXTips
```

---

## TWEET 30 — Week 4 Tuesday (FINAL)
```
30 days. 30 tips.

If you've been following along — you're more prepared than you think.

Full 497-question bank → https://nclexprepro.com
One-time $19. No subscription. Instant access.

Pass NCLEX. Change lives. 💙🏥

#NCLEX #NursingStudents #YouGotThis #NCLEXPrep
```

---

# HOW TO QUEUE IN BUFFER (5 minutes)
1. Go to app.buffer.com
2. Click "New Post" → select @NCLEXPrePro Twitter
3. Copy/paste each tweet
4. Set time: 7:00 PM CST daily
5. Click "Add to Queue" — repeat for all 30

**Status:**
- Tweets 1–5: Already posted ✅
- Tweets 6–14: Queue next (Week 1 Sat → Week 2 Sun)
- Tweets 15–30: Written Mar 8 2026 — queue after 14 posts

Buffer free = 10 posts max in queue. Refill every ~10 days.
